...
const LoginModal = () => {
  ...
  const sendDataRegistration = async (data: {login: string, email: string, password: string}) => {
    try {
      const res = await axios.post("http://35.225.244.52/register/", data);
      navigate('/')
    } catch (err: any){
  ...
  const sendPhoneNumber = async (phone: string) => {
    try {
      ...
      const res = await axios.post("http://35.225.244.52/auth-with-tg/", { phone_number: phone });
  ...
  const verifyOTP = async (phone: string, otp: string) => {
    try {
      const res = await axios.post("http://35.225.244.52/otp-verify/", {
        phone_number: phone,
        code: otp,
      });
...
// const number = full.slice(-9);
  ...
  <form onSubmit={handleRegisterSubmit(sendDataRegistration)}>
    ...
    <div className={styles.log_reg}>
      <button type="submit" className={styles.login}>Зарегистрироваться</button>
      <button
        type="button"
        ...
        Назад
          ...
          {authWay === 'telegram' && (
            <div className={styles.telegramLogin}>
              {telegramWay === 'input' && (
                <form onSubmit={handleTelegramSubmit(({ phone, countryCode }) => sendPhoneNumber(countryCode+phone))}>
          ...
          {/* {showOtp && (
            <form>
              ...
              {...registerTelegram('otp', {
                ...
                minLength: {value: 4, message: 'Min 4 numbers'}
              })}/>
              <button onClick={handleTelegramSubmit(({phone, countryCode, otp}) => {
